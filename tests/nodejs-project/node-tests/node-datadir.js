      ...
      switch (request) {
        case "write-and-read-file":
          // write a file to the writable directory and read it correctly.
          {
            ...
            let writablePath = path.join(
              cordova.app.datadir(),
              "writefile.txt",
            );
            fs.writeFileSync(writablePath, contentToWrite, {
              encoding: "utf8",
            });
            let readResult = fs.readFileSync(writablePath, {
              encoding: "utf8",
            });
            if (contentToWrite !== readResult) {
              result =
                "ko: file contents read are not what was supposed to be written.";
            }
