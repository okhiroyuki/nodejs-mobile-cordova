...
function saveBuildNativeModulesPreference(context, platform) {
  ...
  var saveBuildNativeModulesPreferencePath = path.join(
    wwwPath,
    "NODEJS_MOBILE_BUILD_NATIVE_MODULES_VALUE.txt",
  );
  if (process.env.NODEJS_MOBILE_BUILD_NATIVE_MODULES !== undefined) {
    fs.writeFileSync(
      saveBuildNativeModulesPreferencePath,
      process.env.NODEJS_MOBILE_BUILD_NATIVE_MODULES,
    );
  }
}
